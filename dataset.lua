...
function dataset:getSemantic(i1, i2)
   local indices = torch.range(i1, i2);
   local quantity = i2 - i1 + 1;
   assert(quantity > 0)
   -- now that indices has been initialized, get the samples
   ...
   for i=1,quantity do
      ...
      local out = self:sampleHookTest(imgpath)
      table.insert(dataTable, out)
      table.insert(scalarTable_1, 1)
      table.insert(scalarTable_2, self.imageClass[indices[i]])
   end
   local data, vectors, scalarLabels = tableToSemanticOutput(self, dataTable, torch.rand(quantity,1), scalarTable_1, scalarTable_2)
   return data, scalarLabels
end
